...
static int	ft_in_put_part_int(char *d_i, int save_i, t_flags flags)
{
	...
	if (save_i < 0 && flags.dot >= 0)
		ft_putchar('-');
	if (flags.dot >= 0)
		char_count += process_width(flags.dot - 1, ft_strlen(d_i) - 1, 1);
	char_count += putstrprec(d_i, ft_strlen(d_i));
	return (char_count);
}
...
static int	ft_put_part_int(char *d_i, int save_i, t_flags flags)
{
	...
	if (flags.dot >= 0)
	{
		...
		char_count += process_width(flags.width, ft_strlen(d_i), flags.zero);
	if (flags.minus == 0)
		char_count += ft_in_put_part_int(d_i, save_i, flags);
	return (char_count);
}
